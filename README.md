...
## M25A03. рефакторинг. 3 способа контроля обращения к ресурсу по порту

- 1-й способ: рефакторинг ```SpecialPortFilter``` -> SpecialBravoPortFilter.
описание в [M25901](#M25901)
- 2-й способ: использование АОП. отбор по аннотации ```@SpecialBravoPort```
- 3-й способ: прямая проверка внутри контроллера
