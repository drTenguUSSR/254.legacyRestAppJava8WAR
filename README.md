...
Legacy-приложение на Java 8 для разворачивания на Tomcat(WAR), используя старые библиотеки
...
## <a name="done">Сделано. Сортировка от старых к новым</a>
...
✅ реализация контроля портов: использование АОП (Spring AOP) - 
```SpecialDeltaController```

✅ рефакторинг - убрать лишние cmd из корня проекта

✅ исправление интеграционных тестов для Bravo и Delta

## 🔥 <a name="todo-current">Текущие задачи</a>

- полный параметрический интеграционный тест для Bravo/Delta по фильтрации доступа в
зависимости от порта (main/spec)

## 🎯 <a name="todo-near">TODO ближайшие</a>

- ?WADL для генерации py-client
- ?скрипт cmd-проверки HelloServices - возврат 3х заголовков. ?pyton
- Добавить: Проверка контролера 3-й способ. KiloController - via
```com.sun.jersey.api.core.ResourceMethod``` => 01.multuports.md
- Добавить: Проверка контролера 4-й способ. SpecialLimaController.
Прямая проверка внутри контроллера
...
## 🧾 <a name="todo-far">TODO далее</a>

- упаковка приложения в докерe: в докере устанавливается Linux+Tomcat
в двухпортовй конфигурации
- сервер должен корректно определять запуск в контейнере и выводить
IP адреса с сетевых интерфейсов контейнера 
...
## M25A03. Рефакторинг. Два способа контроля обращения к ресурсу по порту

- **1-й способ: SpecialBravoController**. Рефакторинг ```SpecialPortFilter```
\-> ```SpecialBravoPortFilter```. описание в [M25901](#M25901):
  - при загрузке заполняется Map, где ключ URL (константа), а значение - 
класс контролера (константа)
  - при срабатывании фильтра из Map по URL извлекается класс-контролера. Далее, если у контролера есть
  аннотация ```@SpecialBravoPort``` И порт запроса не является специальным - выбрасывается ошибка
  и контролер не вызывается; иначе - запрос передается в контроллер.
  Недостатки:
    1) пути к контролеру нужно заранее прописывать в коде как константы;
    2) шаблонные пути типа ```@Path("/{some:(?i:foo|bar)}/yoyo")``` не поддерживаются.
- **2-й способ: ```SpecialDeltaController```**. Использование АОП. отбор по аннотации ```@SpecialDeltaPort```.
Реализация в ```SpecialDeltaAspect```. Достоинство - пути к специальному контролеру нигде не фиксированы.
Недостатки - использование АОП.
...
## <a name="M25901">M25901. Контроль доступа через аннотацию</a>
